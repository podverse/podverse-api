    ...
    test('when a valid id is provided', async (done) => {
      chai.request(global.app)
        ...
        .end((err, res) => {
          ...
          // const chapter1 = res.body.chapters[0]
          ...
          // const chapter2 = res.body.chapters[0]

          // chaiExpect(chapter2.endTime).to.equal(360)
          // chaiExpect(chapter2.startTime).to.equal(240)
          // chaiExpect(chapter2.title).to.equal('One more soundbite title')

          // const chapter3 = res.body.chapters[0]

          // chaiExpect(chapter3.endTime).to.equal(632)
          // chaiExpect(chapter3.startTime).to.equal(482)
          // chaiExpect(chapter3.title).to.equal('Another soundbite title')

          // const chapter4 = res.body.chapters[0]

          // chaiExpect(chapter4.endTime).to.equal(735)
          // chaiExpect(chapter4.startTime).to.equal(555)

          // chaiExpect(Object.keys(res.body).length).to.equal(5)
