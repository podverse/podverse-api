      ...
      <body style="margin: 0; padding: 0;">
        <div class="container" style="background-color: #D8D8D8; font-family: 'Arial', sans-serif; margin: 0; padding: 0 0 32px 0;">
          <div class="nav" style="background-color: ${
            config.emailBrandColor
          }; height: 58px; text-align: center; width: 100%;">
            ${
              config.emailHeaderImageUrl
                ? `<img src="${config.emailHeaderImageUrl}" style="height: 38px; margin-top: 10px; max-width: 280px;" />`
                : ''
            }
          </div>
          <div class="content" style="background-color: #FFF; margin: 40px auto; max-width: 380px; padding: 40px 40px 48px 40px;">
            ${
              headerText
                ? `<h1 style="color: ${config.emailBrandColor}; font-size: 30px; margin: 0 0 32px 0; text-align: center;">${headerText}</h1>`
                : ''
            }
            ${
              paragraphText
                ? `<p style="color: #000; font-size: 14px; margin: 0 0 32px 0; text-align: center;">${paragraphText}</p>`
                : ''
            }
            ${
              buttonLink && buttonText
                ? `<a class="button" href="${buttonLink}" style="background-color: ${config.emailBrandColor}; border-radius: 100px; color: #FFF; display: block; font-size: 14px; height: 40px; line-height: 40px; text-align: center; text-decoration: none; width: 100%;">${buttonText}</a>`
                : ''
            }
          </div>
          <div class="footer">
...
const addressSection =
  config.legalName || config.legalAddress
    ? `
...
const facebookIcon =
  config.socialFacebookImageUrl && config.socialFacebookPageUrl
    ? `
...
const githubIcon =
  config.socialGithubImageUrl && config.socialGithubPageUrl
    ? `
...
const redditIcon =
  config.socialRedditImageUrl && config.socialRedditPageUrl
    ? `
...
const twitterIcon =
  config.socialTwitterImageUrl && config.socialTwitterPageUrl
    ? `
