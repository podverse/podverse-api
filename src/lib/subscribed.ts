...
export async function getFollowedPlaylistIdsPrivate(account_id: number): Promise<number[]> {
  const accountFollowingPlaylistService = new AccountFollowingPlaylistService();
  const results = await accountFollowingPlaylistService.getFollowedPlaylistsPrivateWithCount(Number(account_id));
